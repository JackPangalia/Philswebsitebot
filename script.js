...
function saveChatHistory() {
  const messages = Array.from(messageArea.children)
    ...
    .map((element) => {
      ...
      if (type === "chatbot-message-history") type = "chatbot-message";
      if (type === "user-message-history") type = "user-message";
      return {
        type: type,
...
function loadChatHistory() {
  ...
  if (savedHistory) {
    ...
    messages.forEach((message) => {
      ...
      // Replace animation classes with history classes
      const className = message.type === "chatbot-message" 
...
document.addEventListener("visibilitychange", () => {
  if (!document.hidden && chatbotContainer.style.display === "block") {
    setTimeout(scrollToBottom, 100);
  }
});
